...
| _file_systems   | names of different file systems (ext4, ntfs, etc.)                                                        |
...
_regex_arguments or _alternative functions (but is available for _arguments).
** Useful variables
The following variables can be used inside completion functions:
| $service   | the name of the command being completed                                                                              |
| $match     | array containing subexpression matches of previous regexp match using =~                                             |
| $words     | array of words on the command line. Can be altered with compset                                                      |
| $CURRENT   | index into $words of the current word (where the cursor is). Can be altered with compset                             |
| $PREFIX    | part of the current word from the start (unless IPREFIX is non-empty) up to the cursor. Can be altered with compset. |
| $IPREFIX   | part of current word before prefix. Not used for matching with completions. Can be altered with the compset command. |
| $QIPREFIX  | contains quoted string upto word being completed, e.g. if cursor is at a in "foo baa" then QIPREFIX = \"foo b        |
| $QISUFFIX  | like $QIPREFIX but for the end of the quoted string                                                                  |
| $SUFFIX    | part of current word from cursor to the end (unless ISUFFIX is non-empty). Can be altered with compset.              |
| $_comps    | assoc array of commands and corresponding completion functions                                                       |
| $compstate | assoc array containing various keys & values used to exchange information with completion widget                     |
...
You can also allow for an (optional) equals sign to be placed directly between the option and its argument by placing 
an equals sign in the option specification.
For example:
#+BEGIN_SRC sh 
_arguments '-f=[input file]:filename:_files' 
#+END_SRC
All options must begin with either +, - or --. If you want other kinds of options you will need to use a different
function such as _regex_arguments (see next section).
...
You can use quoted parentheses and '|' to separate alternative sequences, and '#' to indicate that the previous
sequence can repeat.
...
_cmd "$@"
#+END_SRC
this specifies a command line matching either repeats of SEQ1, or SEQ2 followed by SEQ2a or SEQ2b.
...
except that it has an extra : at the start, and you cannot use the ->STRING form of ACTION.
...
_regex_arguments _cmd /$'[^\0]##\0'/ \( /$'word1(a|b|c)\0'/ ':word:first word:(word1a word1b word1c)' '|'\
   ...
   '|' /$'word22(a|b|c)\0'/ ':file:file name:_files' \) \)
_cmd "$@"
#+END_SRC
...
Note that each pattern ends with \0, this ensures that there must be whitespace between completions. 
Sometimes you might not want this requirement however (e.g. for options of the form --opt=), in which case you should remove the \0 from the end of the pattern.
...
*** Patterns
Patterns are matched using glob operators as explained here:  http://zsh.sourceforge.net/Doc/Release/Expansion.html#Filename-Generation
...
as a null char which is used to separate words in the internal representation. 

If you don't include the \0 at the end of the pattern then there should be no space between the word matched by the pattern
and the next word (this is sometimes desirable).

If you need to use the contents of a variable in a pattern, you can double quote it so that it gets expanded and then put 
a string parameter containing a null char afterwards, like this: "$somevar"$'\0'

SYNTAX:
| *     | wildcard - any number of chars                                                                      |
| ?     | wildcard - single char                                                                              |
| #     | zero or more of the previous pattern (like * in a normal regular expression)                        |
| ##    | one or more of the previous pattern (like + in a normal regular expression)                         |
| [..]  | matches any of the enclosed chars, can be a range (e.g. [a-z]) or a named class (e.g. [[:digit:] ]) |
| [!..] | match any char NOT in the given set                                                                 |
| <X-Y> | match any number between X and Y                                                                    |
| (..)  | grouping contruct                                                                                   |
| ^     | negate the following pattern                                                                        |
| X~Y   | match X but not Y                                                                                   |
For more details, see here: http://zsh.sourceforge.net/Doc/Release/Expansion.html#Filename-Generation

EXAMPLES:
/$'(foo|bar)\0'/ = matches either foo or bar
/$'[ [:alnum:]]##\0'/ = matches a sequence of one or more letters or numbers
/$'[![:alnum:]]##\0'/ = matches a sequence of one or more chars that are not letters or numbers
/$'[^\0]##\0'/ = matches any unbroken sequence of chars
*** _regex_words
...
word to be completed, DESCRIPTION is a description for it (must be non-empty), and SPEC can be another variable created by _regex_words 
...
Here are some useful techniques for completing files and directories.

Complete any file (starts in current directory):
#+BEGIN_SRC
_files
#+END_SRC

Complete any directory (starts in current directory):
#+BEGIN_SRC
_path_files -/ 
#+END_SRC

Complete files in a specific directory (must put / at end of paths):
#+BEGIN_SRC
_files -P /path/to/dir/ -W /path/to/dir/
#+END_SRC

Complete .csv files in specific directory:
#+BEGIN_SRC
_path_files -P /path/to/dir/ -W /path/to/dir/ -g '*.csv'
#+END_SRC

Complete files in specific directory, but don't add that directory to the command line:
#+BEGIN_SRC
_files "$@" -W /path/to/dir/
#+END_SRC

Complete .csv files in specific directory, but don't add that directory to the command line:
#+BEGIN_SRC
_path_files "$@" -W /path/to/dir/ -g '*.csv'
#+END_SRC


Complete comma separated files:
#+BEGIN_SRC 
compset -P '*,'
_files -S ,
#+END_SRC

Complete comma separated directories:
#+BEGIN_SRC 
compset -P '*,'
_path_files -/ -S ,
#+END_SRC
Note: the previous two examples use the compset builtin which allows you to manipulate how the shell treats completions. 
You can use this trick with other functions aswell, or for testing the prefix/suffix of the current word on the command line.
...
> autoload -U _func
#+END_SRC
To reload all functions matching a pattern (in this case functions with names beginning with "_func_"):
#+BEGIN_SRC sh 
> unfunction -m _func\*
> autoload -U _func
#+END_SRC
...
* Gotchas (things to watch out for)
 ...
 - Do not include any spaces around equals symbols used in variable/array assignments.

 - Make sure to quote parentheses and vertical bars used for bracketing arguments to _regex_arguments
 ...
 - Make sure you quote any asterisks (*) and colons (:) in option name and descriptions arguments to _alternatives, _regex_words, etc.
